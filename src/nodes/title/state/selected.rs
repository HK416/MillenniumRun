...
fn handle_mouse_input_for_ui(this: &mut TitleScene, shared: &mut Shared, event: &Event<AppEvent>) -> AppResult<()> {
    use std::sync::Mutex;
    static FOCUSED: Mutex<Option<(ty::StageWindowTags, Vec3, Vec<Vec3>)>> = Mutex::new(None);
        ...
        Event::WindowEvent { event, .. } => match event {
            WindowEvent::MouseInput { state, button, .. } => {
                if MouseButton::Left == *button && state.is_pressed() {
                    ...
                    let pickable = this.stage_window.pickable();
                    ...
                    if let Some((tag, ui_color, text_colors)) = guard.take() {
                        ...
                        if let Some((ui, texts)) = this.stage_window.get_mut(tag as usize) {
                            ui.update_buffer(queue, |data| {
        ...
        Event::WindowEvent { event, .. } => match event {
                ...
                if MouseButton::Left == *button && state.is_pressed() {
                    ...
                    if let Some((tag, (ui, texts))) = select {
                        ...
                        *guard = Some((tag, ui_color, text_colors));
                        ...
                        // <3>
                        sys_ui_pressed(tag, this, shared)?;
                    }
                } else if MouseButton::Left == *button && !state.is_pressed() {
                    let mut guard = FOCUSED.lock().expect("Failed to access variable.");
                    if let Some((tag, ui_color, text_colors)) = guard.take() {
                        ...
                        if select.is_some_and(|select| tag == select) {
                            // (한국어) ui 떼어짐 함수를 호출합니다.
                            // (English Translation) Calls the ui released function.
                            sys_ui_released(tag, this, shared)?;
                        }
                    }
                }
            },
            WindowEvent::CursorMoved { .. } => {
                ...
                if let Some((tag, _, _)) = guard.as_ref() {
                    // (한국어) ui 끌림 함수를 호출합니다.
                    // (English Translation) Calls the ui dragged function.
                    sys_ui_dragged(*tag, this, shared)?;
                }
            },
            _ => { /* empty */ }
        },
...
fn ui_pressed(tag: ty::StageWindowTags, this: &mut TitleScene, shared: &mut Shared) -> AppResult<()> {
    match tag {
        ty::StageWindowTags::Enter => {
...
#[allow(unused_variables)]
#[allow(unreachable_patterns)]
fn sys_ui_pressed(tag: ty::SystemButtonTags, this: &mut TitleScene, shared: &mut Shared) -> AppResult<()> {
    match tag {
...
#[allow(unused_variables)]
#[allow(unreachable_patterns)]
fn ui_released(tag: ty::StageWindowTags, this: &mut TitleScene, _shared: &mut Shared) -> AppResult<()> {
    match tag {
        _ => Ok(())
    }
}
...
#[allow(unused_variables)]
#[allow(unreachable_patterns)]
fn sys_ui_released(tag: ty::SystemButtonTags, this: &mut TitleScene, _shared: &mut Shared) -> AppResult<()> {
    match tag {
        ty::SystemButtonTags::ReturnButton => {
            this.state = TitleState::ExitSelected;
            this.elapsed_time = 0.0;
...
#[allow(unused_variables)]
#[allow(unreachable_patterns)]
fn ui_dragged(tag: ty::StageWindowTags, this: &mut TitleScene, shared: &mut Shared) -> AppResult<()> {
    match tag {
        _ => Ok(())
    }
}
...
#[allow(unused_variables)]
#[allow(unreachable_patterns)]
fn sys_ui_dragged(tag: ty::SystemButtonTags, this: &mut TitleScene, shared: &mut Shared) -> AppResult<()> {
    match tag {
        _ => Ok(())
    }
}
