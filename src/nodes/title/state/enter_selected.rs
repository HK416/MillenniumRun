...
/// #### 한국어 </br>
/// `EnterSelected`상태의 지속 시간입니다. </br>
/// 
/// #### English (Translation) </br>
/// Duration of the `EnterSelected` state. </br>
/// 
...
pub fn update(this: &mut TitleScene, shared: &mut Shared, _total_time: f64, elapsed_time: f64) -> AppResult<()> {
    ...
    let ortho = Orthographic {
        top: DEF_PROJECTION.top + (FOCUSED_PROJECTIONS[tag as usize].top - DEF_PROJECTION.top) * delta,
        left: DEF_PROJECTION.left + (FOCUSED_PROJECTIONS[tag as usize].left - DEF_PROJECTION.left) * delta,
        bottom: DEF_PROJECTION.bottom + (FOCUSED_PROJECTIONS[tag as usize].bottom - DEF_PROJECTION.bottom) * delta,
        right: DEF_PROJECTION.right + (FOCUSED_PROJECTIONS[tag as usize].right - DEF_PROJECTION.right) * delta,
        near: DEF_PROJECTION.near + (FOCUSED_PROJECTIONS[tag as usize].near - DEF_PROJECTION.near) * delta,
        far: DEF_PROJECTION.far + (FOCUSED_PROJECTIONS[tag as usize].far - DEF_PROJECTION.far) * delta,
    };
    ...
    if this.elapsed_time >= DURATION {
        this.state = TitleState::Selected;
