...
pub fn draw(this: &TitleScene, shared: &mut Shared) -> AppResult<()> {
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(Tutorial0(Background)))"),
    ...
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(Tutorial0(Ui)))"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: &view,
                resolve_target: None,
                ops: wgpu::Operations {
        ...
        camera.bind(&mut rpass);
        ui_brush.draw(&mut rpass, [&this.tutorials[0].0, &this.tutorial_next_btn].into_iter());
        text_brush.draw(&mut rpass, [&this.tutorials[0].1].into_iter());
    }
        ...
        Event::WindowEvent { event, .. } => match event {
            WindowEvent::MouseInput { state, button, .. } => {
                if MouseButton::Left == *button && state.is_pressed() {
                    ...
                    let selected = this.tutorial_next_btn.test(&(cursor_pos, camera));
                    if selected {
                        // (한국어) 선택된 인터페이스의 데이터를 저장합니다.
                        // (English Translation) Stores the data of the selected interface. 
                        let ui_color = this.tutorial_next_btn.data.lock().expect("Failed to access variable.").color.xyz();
                        let mut guard = FOCUSED_UI.lock().expect("Failed to access variable.");
                        *guard = Some((Buttons::Next, ui_color));

                        // (한국어) 선택된 인터페이스를 갱신합니다.
                        // (English Translation) Updates the selected interface. 
                        this.tutorial_next_btn.update(queue, |data| data.color *= Vec4::new(0.5, 0.5, 0.5, 1.0));
