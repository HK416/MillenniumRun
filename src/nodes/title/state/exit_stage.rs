...
/// #### 한국어 </br>
/// `ExitStage` 상태의 지속 시간입니다. </br>
/// 
/// #### English (Translation) </br>
/// Duration of the `ExitStage` state. </br>
/// 
const DURATION: f64 = 1.5;

const MOVING_TIME: f64 = DURATION;
const SHOW_TIME: f64 = 0.5 * DURATION;
...
pub fn update(this: &mut TitleScene, shared: &mut Shared, _total_time: f64, elapsed_time: f64) -> AppResult<()> {
    ...
    let delta = delta_time((this.elapsed_time - SHOW_TIME).max(0.0), SHOW_TIME).min(1.0);
    let menu_alpha = 1.0 * delta;
    let delta = delta_time(this.elapsed_time, MOVING_TIME).min(1.0);
    let stage_alpha = 1.0 - 1.0 * delta;
    let position = STAGE_CAMERA_POS + (MENU_CAMERA_POS - STAGE_CAMERA_POS) * delta;
...
pub fn draw(this: &TitleScene, shared: &mut Shared) -> AppResult<()> {
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(ExitStage(Background)))"),
    ...
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(ExitStage(Ui)))"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: &view,
                resolve_target: None,
                ops: wgpu::Operations {
...
fn delta_time(elapsed_time: f64, duration: f64) -> f32 {
    let t = (elapsed_time.clamp(0.0, duration) / duration) as f32;
...
fn update_camera_pos(
    camera: &mut GameCamera, position: Vec3, queue: &wgpu::Queue) {
