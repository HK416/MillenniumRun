...
pub fn draw(this: &TitleScene, shared: &mut Shared) -> AppResult<()> {
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(Tutorial1(Background)))"),
    ...
    {
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderPass(TitleScene(Tutorial1(Ui)))"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: &view,
                resolve_target: None,
                ops: wgpu::Operations {
        ...
        camera.bind(&mut rpass);
        ui_brush.draw(&mut rpass, [&this.tutorials[1].0, &this.tutorial_prev_btn, &this.tutorial_next_btn].into_iter());
        text_brush.draw(&mut rpass, [&this.tutorials[1].1].into_iter());
    }
                    ...
                    if let Some((btn, ui_color)) = guard.take() {
                        ...
                        if selected.is_some_and(|selected| selected == btn) {
                            ...
                            match btn {
                                ...
                                Buttons::Next => {
                                    this.timer = 0.0;
                                    this.state = TitleState::Tutorial2;
                                }
                            };
                        }
                    }
