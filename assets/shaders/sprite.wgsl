struct VertexInput {
    ...
    @location(4) uv_top: f32,
    @location(5) uv_left: f32,
    @location(6) uv_bottom: f32,
    @location(7) uv_right: f32,
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    switch (in.vertex_index) {
        case 0u: {
            ...
            texcoord = vec2<f32>(in.uv_left, in.uv_bottom);
            break;
        }
        case 1u: {
            position = vec3<f32>(-0.5 * in.size.x, 0.5 * in.size.y, 0.0);
            texcoord = vec2<f32>(in.uv_left, in.uv_top);
            break;
        }
        case 2u: {
            position = vec3<f32>(0.5 * in.size.x, -0.5 * in.size.y, 0.0);
            texcoord = vec2<f32>(in.uv_right, in.uv_bottom);
            break;
        }
        case 3u: {
            position = vec3<f32>(0.5 * in.size.x, 0.5 * in.size.y, 0.0);
            texcoord = vec2<f32>(in.uv_right, in.uv_top);
            break;
        }
        default { }
    }
